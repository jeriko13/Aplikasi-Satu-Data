...
export default function KinerjaBidangTable({id}) {
    const [dataKinerjaOpd, setDataKinerjaOpd] = useState([])

    useEffect(() => {
      (async () => {
        const kinerjaOpd = await Axios.get(`${API_URL}/api/list/cv_listkinerjabidang`).then(res => res.data.cv_listkinerjabidang);
