...
export default function ResDanaPembangunanDesa({tahun}) {
  const [dataDana, setDataDana] = useState([])

  useEffect(() => {
  (async () => {
      const resDana = await Axios.get(`${API_URL}/api/list/res_dana_pembangunan_desa?cmd=search&t=res_dana_pembangunan_desa&z_tahun=LIKE&x_tahun=${tahun}`).then(res => res.data.res_dana_pembangunan_desa);
      setDataDana(resDana)
    })()
  }, []);
