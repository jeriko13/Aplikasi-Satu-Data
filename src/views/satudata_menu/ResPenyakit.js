...
export default function ResPenyakit({tahun}) {
    const [dataPenyakit, setDataPenyakit] = useState([])

    useEffect(() => {
      (async () => {
        const resPenyakit = await Axios.get(`${API_URL}/api/list/res_penyakit?cmd=search&t=res_penyakit&z_tahun=LIKE&x_tahun=${tahun}`).then(res => res.data.res_penyakit);
        setDataPenyakit(resPenyakit)
      })()
    }, []);
          ...
          <div className="flex flex-wrap items-center">
            <div className="relative w-full px-4 max-w-full flex-grow flex-1 flex justify-between items-center">
              <h3 className="font-semibold text-base text-blueGray-700">
                Pelayanan Penyakit tahun {tahun}
              </h3>
