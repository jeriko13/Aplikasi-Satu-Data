...
export default function ResPendidikan({tingkatPendidikan, tahun}) {
    const [dataPendidikan, setDataPendidikan] = useState([])

    useEffect(() => {
    (async () => {
        const resPendidikan = await Axios.get(`${API_URL}/api/list/res_pendidikan?cmd=search&t=res_pendidikan&z_jenis_sekolah=LIKE&x_jenis_sekolah=${tingkatPendidikan}&z_tahun=LIKE&x_tahun=${tahun}`).then(res => res.data.res_pendidikan);
          ...
          <div className="flex flex-wrap items-center">
            <div className="relative w-full px-4 max-w-full flex-grow flex-1 flex justify-between items-center">
              <h3 className="font-semibold text-base text-blueGray-700">
                Pendidikan (Jumlah Sekolah, Guru, dan Murid {tingkatPendidikan}) tahun {tahun}
              </h3>
