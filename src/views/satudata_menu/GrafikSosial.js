...
export default function GrafikSosial() {
    useEffect(() => {
      (async () => {
        const resSosial = await Axios.get(`${API_URL}/api/list/grafik_sosial`).then(res => res.data.grafik_sosial);
        let config = {
        type: "bar",
        data: {
            labels: resSosial.map(i => i.tahun),
            datasets: [
            {
                label: "Penerima Bantuan",
                backgroundColor: "#ed64a6",
                borderColor: "#ed64a6",
                data: resSosial.map(i => parseFloat(i.penerima_bantuan)),
                fill: false,
                barThickness: 20,
            },
            {
                label: "Anggaran Bantuan Sosial Pangan",
                fill: false,
                backgroundColor: "#4c51bf",
                borderColor: "#4c51bf",
                data:  resSosial.map(i => parseFloat(i.anggaran_bantuan_sosial_pangan)),
                barThickness: 20,
            },
