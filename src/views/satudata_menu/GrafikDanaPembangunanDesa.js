...
export default function GrafikDanaPembangunanDesa() {
    useEffect(() => {
      (async () => {
        const resDanaPembangunanDesa = await Axios.get(`${API_URL}/api/list/grafik_dana_pembangunan_desa`).then(res => res.data.grafik_dana_pembangunan_desa);
        let config = {
        type: "bar",
        data: {
            labels: resDanaPembangunanDesa.map(i => i.tahun),
            datasets: [
            {
                label: "Dana Pembangunan Desa",
                backgroundColor: "#ed64a6",
                borderColor: "#ed64a6",
                data: resDanaPembangunanDesa.map(i => parseFloat(i.dana_pembangunan_desa)),
                fill: false,
                barThickness: 20,
            },
