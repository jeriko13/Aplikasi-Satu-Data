...
export default function GrafikPendidikanSmp() {
    useEffect(() => {
      (async () => {
        const resPendidikan = await Axios.get(`${API_URL}/api/list/grafik_pendidikan_smp`).then(res => res.data.grafik_pendidikan_smp);
        let config = {
        type: "bar",
        data: {
            labels: resPendidikan.map(i => i.tahun),
            datasets: [
            {
                label: "Sekolah Negeri",
                backgroundColor: "#ed64a6",
                borderColor: "#ed64a6",
                data: resPendidikan.map(i => parseFloat(i.sekolah_negeri)),
                fill: false,
                barThickness: 20,
            },
            {
