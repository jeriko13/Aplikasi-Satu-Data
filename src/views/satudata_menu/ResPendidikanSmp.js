...
export default function ResPendidikanSmp({tingkatPendidikan, tahun}) {
    const [dataPendidikan, setDataPendidikan] = useState([])

    useEffect(() => {
    (async () => {
        const resPendidikan = await Axios.get(`${API_URL}/api/list/res_pendidikan_smp?cmd=search&t=res_pendidikan_smp&z_jenis_sekolah=LIKE&x_jenis_sekolah=${tingkatPendidikan}&z_tahun=LIKE&x_tahun=${tahun}`).then(res => res.data.res_pendidikan_smp);
        ...
        <div className="rounded-t mb-0 px-4 py-3 border-0">
          <div className="flex flex-wrap items-center">
            <div className="relative w-full px-4 max-w-full flex-grow flex-1 flex justify-between items-center">
              ...
              Jumlah Sekolah, Guru, dan Murid Sekolah Menengah Pertama (SMP) Tahun {tahun}
              </h3>
