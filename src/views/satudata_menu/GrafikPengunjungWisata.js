...
export default function GrafikPengunjungWisata() {
    useEffect(() => {
      (async () => {
        const resPengunjungWisata = await Axios.get(`${API_URL}/api/list/grafik_pengunjung_wisata`).then(res => res.data.grafik_pengunjung_wisata);
        let config = {
        type: "bar",
        data: {
            labels: resPengunjungWisata.map(i => i.tahun),
            datasets: [
            {
                label: "Jumlah Pengunjung",
                backgroundColor: "#ed64a6",
                borderColor: "#ed64a6",
                data: resPengunjungWisata.map(i => parseFloat(i.jumlah_pengunjung)),
                fill: false,
                barThickness: 20,
            },
