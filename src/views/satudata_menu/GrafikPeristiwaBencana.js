...
export default function GrafikPeristiwaBencana() {
    useEffect(() => {
      (async () => {
        const resPeristiwaBencana = await Axios.get(`${API_URL}/api/list/grafik_peristiwa_bencana`).then(res => res.data.grafik_peristiwa_bencana);
        let config = {
        type: "bar",
        data: {
            labels: resPeristiwaBencana.map(i => i.tahun),
            datasets: [
            {
                label: "Tanah Longsor",
                backgroundColor: "#ed64a6",
                borderColor: "#ed64a6",
                data: resPeristiwaBencana.map(i => parseFloat(i.tanah_longsor)),
                fill: false,
                barThickness: 20,
            },
            {
                ...
                borderColor: "#4c51bf",
                data:  resPeristiwaBencana.map(i => parseFloat(i.angin_kencang)),
                barThickness: 20,
            },
            {
                label: "Kebakaran",
                ...
                borderColor: "#378aa3",
                data: resPeristiwaBencana.map(i => parseFloat(i.kebakaran)),
                fill: false,
                barThickness: 20,
            },
