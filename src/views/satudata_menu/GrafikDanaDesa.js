...
export default function GrafikDanaDesa() {
    useEffect(() => {
      (async () => {
        const resDanaDesa = await Axios.get(`${API_URL}/api/list/grafik_dana_desa`).then(res => res.data.grafik_dana_desa);
        let config = {
        type: "bar",
        data: {
            labels: resDanaDesa.map(i => i.tahun),
            datasets: [
            {
                label: "Alokasi Dana",
                backgroundColor: "#ed64a6",
                borderColor: "#ed64a6",
                data: resDanaDesa.map(i => parseFloat(i.alokasi_dana)),
                fill: false,
                barThickness: 20,
            },
