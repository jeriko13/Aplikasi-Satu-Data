...
export default function GrafikKelahiran() {
    useEffect(() => {
      (async () => {
        const resKelahiran = await Axios.get(`${API_URL}/api/list/grafik_kelahiran`).then(res => res.data.grafik_kelahiran);
        let config = {
        type: "bar",
        data: {
            labels: resKelahiran.map(i => i.tahun),
            datasets: [
            {
                label: "Akta Kelahiran Baru",
                backgroundColor: "#ed64a6",
                borderColor: "#ed64a6",
                data: resKelahiran.map(i => parseFloat(i.akta_kelahiran_baru)),
                fill: false,
                barThickness: 20,
            },
            {
                label: "Akta Kelahiran Terlambat Pencatatan",
                fill: false,
                backgroundColor: "#4c51bf",
                borderColor: "#4c51bf",
                data:  resKelahiran.map(i => parseFloat(i.akta_kelahiran_terlambat_pencatatan)),
                barThickness: 20,
            },
